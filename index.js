...
function removeLastElement(arr) {
  ...
  if (arr.length === 0) {
    // Якщо порожній, повертаємо undefined
    return undefined;
  }
...
function createArray(...elements) {
  ...
  // return elements.length > 0 ? Array.of(...elements) : [];
...
function customIncludes(arr, element) {
  if (!Array.isArray(arr)) {
    console.log("Помилка: Вхідний параметр не є масивом.");
    return false;
  }

  const includesResult = arr.includes(element);

  const occurrences = arr.filter((el) => el === element).length;
